class Graph:
    ...
    def load(self, initial_data:dict):
        ...
        for v in self.G:
            ...
            self.V.update(
                neighbor_v)  # neighbor_v aka neighbor vertices of :v # add list to a set ref. https://stackoverflow.com/a/43746275/248616
        # endregion
    ...
    def add_edges(self, e:'set(v, nv) as new_edge_v_nv'):
        ...
        # update G
        if self.G.get(v): self.G[v].append(nv)
        else:             self.G[v] = [nv]
        if self.G.get(nv): self.G[nv].append(v)
        else:              self.G[nv] = [v]
